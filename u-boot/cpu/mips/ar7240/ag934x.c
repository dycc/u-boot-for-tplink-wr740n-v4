...
void ag7240_miiphy_write(char *devname, uint32_t phaddr, uint8_t reg,
		uint16_t data);
...
static int ag7240_send(struct eth_device *dev, volatile void *packet,
		int length) {
	int i;
...
void ag7240_mii_setup(ag7240_mac_t *mac) {
	...
	if ((ar7240_reg_rd(AR7240_REV_ID) & AR7240_REV_ID_MASK) == AR7240_REV_1_2) {
		...
		if (mac->mac_unit == 0) {
			ag7240_reg_wr(mac, AG7240_MAC_MII_MGMT_CFG,
					mgmt_cfg_val | (1 << 31));
			ag7240_reg_wr(mac, AG7240_MAC_MII_MGMT_CFG, mgmt_cfg_val);
		}
	} else {
		...
		if ((is_ar7241() || is_ar7242())) {
			...
			/* Virian */
			mgmt_cfg_val = 0x4;
			ag7240_reg_wr(ag7240_macs[1], AG7240_MAC_MII_MGMT_CFG,
					mgmt_cfg_val | (1 << 31));
			ag7240_reg_wr(ag7240_macs[1], AG7240_MAC_MII_MGMT_CFG, mgmt_cfg_val);
...
static void ag7240_hw_start(ag7240_mac_t *mac) {
	...
	if (mac->mac_unit) {
		...
		ag7240_reg_rmw_set(mac, AG7240_MAC_CFG2,
				(AG7240_MAC_CFG2_PAD_CRC_EN | AG7240_MAC_CFG2_LEN_CHECK | AG7240_MAC_CFG2_IF_1000));
	} else {
		ag7240_reg_wr(mac, AG7240_MAC_CFG1,
				(AG7240_MAC_CFG1_RX_EN | AG7240_MAC_CFG1_TX_EN));
		ag7240_reg_rmw_set(mac, AG7240_MAC_CFG2,
				(AG7240_MAC_CFG2_PAD_CRC_EN | AG7240_MAC_CFG2_LEN_CHECK | AG7240_MAC_CFG2_IF_10_100));
	}
...
int ag7240_enet_initialize(bd_t * bis) {
	...
	for (i = 0; i < CFG_AG7240_NMACS; i++) {
		if ((dev[i] = (struct eth_device *) malloc(sizeof(struct eth_device)))
				== NULL) {
			//puts("malloc failed\n");
			return (0);
		}

		if ((ag7240_macs[i] = (ag7240_mac_t *) malloc(sizeof(ag7240_mac_t)))
				== NULL) {
			//puts("malloc failed\n");
			return (0);
		}
	...
	for (i = 0; i < CFG_AG7240_NMACS; i++) {
		...
		ag7240_reg_rmw_set(ag7240_macs[i], AG7240_MAC_CFG1,
				AG7240_MAC_CFG1_SOFT_RST | AG7240_MAC_CFG1_RX_RST | AG7240_MAC_CFG1_TX_RST);
...
void ag7240_miiphy_write(char *devname, uint32_t phy_addr, uint8_t reg,
		uint16_t data) {
	ag7240_mac_t *mac = ag7240_name2mac(devname);
